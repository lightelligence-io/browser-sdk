...
describe('Event module', () => {
  test('getEvents calls ApiService', () =>
    Event.getEvents('1').then(() => expect(ApiService.call).toBeCalledWith('/devices/1/events?')));

  test('getEvents calls ApiService with correct params', () =>
    Event.getEvents('1', {
      eventTypes: 'a,b,c',
    }).then(() =>
      expect(ApiService.call).toBeCalledWith('/devices/1/events?eventTypes=a%2Cb%2Cc'),
    ));
